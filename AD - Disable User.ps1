...
# Type: New Service
#
# Title: AD - Disable User Account
#
# v 1.0 Initial Release
# v 1.1 Added Stored Credentials
#       see for details: https://au2mator.com/documentation/powershell-credentials/?utm_source=github&utm_medium=social&utm_campaign=AD_DisableUser&utm_content=PS1
...
# Code Template V 1.3
#
# URL: https://au2mator.com/disable-user-account-active-directory-self-service-with-au2mator/?utm_source=github&utm_medium=social&utm_campaign=AD_DisableUser&utm_content=PS1
...
[string]$LogfileName = "Disable User"

[string]$CredentialStorePath = "C:\_SCOworkingDir\TFS\PS-Services\CredentialStore" #see for details: https://au2mator.com/documentation/powershell-credentials/?utm_source=github&utm_medium=social&utm_campaign=AD_DisableUser&utm_content=PS1
...
# Stored Credentials
# See: https://au2mator.com/documentation/powershell-credentials/?utm_source=github&utm_medium=social&utm_campaign=AD_DisableUser&utm_content=PS1
...
Write-au2matorLog -Type INFO -Text "Try to disbale AD User"

try {
    if ($c_CheckImmediate -eq "True")
    {
        Disable-ADAccount -Identity $c_User -ErrorAction Stop
        Write-au2matorLog -Type INFO -Text "User disabled"

        $au2matorReturn = "User $((Get-ADUser -identity $c_User).Name) disabled"
        $AdditionalHTML = "<br>
            User $((Get-ADUser -identity $c_User).Name) has been disabled
            <br>
            "
        $Status = "COMPLETED"
    }
    ...
    {
        $exp = Get-Date -Date $c_ExpireDate
        Set-ADAccountExpiration -Identity $C_User -DateTime $exp -ErrorAction Stop
        Write-au2matorLog -Type INFO -Text "User set to expire"

        $au2matorReturn = "User $((Get-ADUser -identity $c_User).Name) set to expire on $c_ExpireDate"
        $AdditionalHTML = "<br>
            User $((Get-ADUser -identity $c_User).Name) set to expire on $c_ExpireDate
            <br>
            "
        $Status = "COMPLETED"
    }
    ...
    Write-au2matorLog -Type ERROR -Text "not able to disbale User"
    Write-au2matorLog -Type ERROR -Text $Error

    $au2matorReturn = "Failed to disable User $c_User, Error: $Error"
    $AdditionalHTML = "<br>
            Failed to disable User $c_User
            <br>
            Error: $Error
            "
    $Status = "ERROR"
